...
1. **[보고서 초안 `ML보고서_초안_대단원3.md`](대단원3_보고서작성/ML보고서_초안_대단원3.md)** — 문제 정의부터 모델 선정·평가·한계까지 전체 서술. 이 저장소의 결론이 다 들어 있다.
2. **그림 3장** — [F1 ΔP 물리 방향성](figures/F1_dp_physical_direction.png) · [F2 ΔP parity](figures/F2_dp_parity.png) · [F3 전환율 parity](figures/F3_conversion_parity.png)
3. **아래 §4 재현 방법** — 배포 ONNX를 직접 돌려 성능 수치를 재계산해 볼 수 있다.
4. 더 깊이 보려면 → [`2026-07-21_ML개발/docs/`](2026-07-21_ML개발/docs/) 개발 과정 기록(모델 비교·시행착오 전과정), [`2026-07-21_ML개발/src/`](2026-07-21_ML개발/src/) 학습 파이프라인 코드
...
**배포 계수** — ONNX2: 절편 −51.7101 · b_T 0.084977 · b_총유량 0.00314476 · b_FRC −0.94009 (§3.6.2). ONNX1: `logit(X) = −53.5111 + 0.135217·TT-1006 + 0.0017134·FT-1004 + 0.525816·FRC-1004` (§3.6.1).

## 3. 폴더 · 핵심 파일

| 경로 | 내용 |
|---|---|
| `대단원3_보고서작성/ML보고서_초안_대단원3.md` | **정본 초안**. 대단원 3(ML) 본문 + 부록 |
| `대단원3_보고서작성/부록_재현_명세.md` | 부록 표 C1–C4·그림 F1–F3의 재현 명세(어느 스크립트·목표 수치) |
| `2026-07-21_ML개발/onnx/reactor_conversion_r1.onnx`<br>`2026-07-21_ML개발/onnx/reactor_dp.onnx` | **배포 모델 2종**. 보고서에서 `onnx/…`로 줄여 쓰는 파일 |
| `대단원3_보고서작성/요청 자료/reactor_*.onnx` | 위 두 파일의 사본 (내용 동일, md5 일치 확인) |
| `ML_마스터데이터_PV.csv` | 학습·검증 마스터 데이터. 3,066행 × 20열, 10/12–10/18 1분 간격 |
...
재현 스크립트(루트): `permutation_importance_검증.py`(표 C4 순열 중요도) · `압력변수_제외근거_검증.py` · `지연효과_검증.py` · `ML_마스터데이터_PV_생성.py` · `figures/make_figures.py`.
