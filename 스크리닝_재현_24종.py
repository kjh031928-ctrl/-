# -*- coding: utf-8 -*-
"""§3.5.1 스크리닝 표(24종) 전체 재현 + 트리 계열 365°C clamp 실측 [부록 A]

무엇을 하는가
  1. 전환율(ONNX1) 후보 24종의 held-out RMSE 를 **전부 다시 계산**한다.
     기존 22종은 `stage1_models.build_specs()` 를 그대로 불러오고, XGBoost·CatBoost 두 종만
  ...
  2. 27점 격자에서 **365°C 예측이 350°C 와 같은지(clamp)** 를 모든 후보에 대해 실측한다.
     ...
     모델별 측정으로 뒷받침하기 위함이다.
  3. 압력강하(ONNX2)에서 XGBoost·CatBoost 를 선형과 비교한다(부록 A).
...
재현
  python 스크리닝_재현_24종.py      (repo 루트에서 실행. 수 분 소요 — GaussianProcess·SVR 포함)
"""
...
SEED = ev.SEED
OUT_JSON = ROOT / "results" / "screening_24models.json"

# 보고서 §3.5.1 표 기재값 (재현 대조용)
REF_RMSE = {
    "Ridge Poly2": 0.01929, "Linear 1차": 0.02099, "PLS": 0.02099, "Poly 2차": 0.02111,
    "Ridge 1차": 0.02232, "SVR(linear)": 0.02353, "ElasticNet": 0.02925, "Lasso": 0.03636,
    "MLP(128, 128)": 0.03680, "MLP(16,)": 0.04527, "MLP(64, 64)": 0.05171, "SVR(RBF)": 0.05873,
    "ExtraTrees": 0.07877, "KNN": 0.07894, "GradientBoosting": 0.07940,
    "XGBoost": 0.08269, "CatBoost": 0.09267, "HistGradientBoosting": 0.09371,
    "RandomForest": 0.09970, "DecisionTree": 0.10052, "AdaBoost": 0.10810,
    "GaussianProcess": 0.16397, "Dummy(mean)": 0.18537, "Poly 3차": 0.37714,
}
TOL = 5e-4
FAILURES: list[str] = []
...
print(f"후보 {len(specs)}종 (기존 {len(specs) - 2} + XGBoost·CatBoost)")
...
# ---------------------------------------------------------------- 1. 전환율 24종
print("=" * 96)
print("1. 전환율(ONNX1) 24종 — held-out RMSE 재현 + 365°C clamp 실측")
print("=" * 96)
...
# ---------------------------------------------------------------- 3. ΔP 비교
print("\n" + "=" * 96)
print("3. 압력강하(ONNX2) — XGBoost·CatBoost 대 선형 (그림 규약과 동일 분할)")
print("=" * 96)
...
REF_DP = {"XGBoost": (0.3421, 3), "CatBoost": (1.0329, 3), "Linear": (0.1910, 9)}
dp_rows = []
...
for key, fac, grid in (("XGBoost", xgb_factory, XGB_GRID),
    ...
    print(f"{key:<12}{R:>15.4f}{ref_r:>10.4f}{str(fl) + '/9':>12}{str(bp) + '/9':>12}")

# ---------------------------------------------------------------- 저장 · 판정
OUT_JSON.parent.mkdir(exist_ok=True)
OUT_JSON.write_text(json.dumps({
    "설명": "§3.5.1 스크리닝 24종 전체 재현 + 365°C clamp 실측",
    "규약": {
...
if FAILURES:
    ...
    for f in FAILURES:
        print("  -", f)
    sys.exit(1)
print(f"24종 전부 기준값 재현됨 · clamp 는 모델별 실측값.")
print("=" * 96)
